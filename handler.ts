import { cloudtrailToParquet } from './index';
 ...
 */
function resolveDate(eventTime: string): { year: number; month: number; day: number } {
    const override = process.env.PROCESS_DATE;
    if (override) {
        const [y, m, d] = override.split('-').map(Number);
        if (!y || !m || !d) throw new Error(`Invalid PROCESS_DATE: ${override} — expected YYYY-MM-DD`);
...
export async function handler(event: ScheduledEvent, context: LambdaContext): Promise<void> {
    ...
    if (!basePath) throw new Error('Missing env var: CLOUDTRAIL_BASE_PATH (e.g. s3://my-bucket/prefix/)');
    if (!outputPath) throw new Error('Missing env var: OUTPUT_PATH (e.g. s3://my-output-bucket/parquet/)');

    // ── Optional env vars ──────────────────────────────────────────────────────
    const organisationId = process.env.ORGANISATION_ID ?? null; // null = no org prefix

    const { year, month, day } = resolveDate(event.time);
    console.log(`Processing CloudTrail logs for ${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`);

    await cloudtrailToParquet(basePath, organisationId, year, month, day, outputPath);
